...
namespace Hikvision
{
    ...
    public class CHCNetSDK
    {
        ...
        static CHCNetSDK()
        {
			...
			path = System.IO.Path.Combine(path, is64 ? "x64" : "x86");

			var ok = SetDllDirectory(path);
			if (!ok)
				throw new System.ComponentModel.Win32Exception($"Unable to set dll directory {path}");
        }
